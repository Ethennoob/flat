...
function db( $host = null , $port = null , $user = null , $password = null , $db_name = null )
{
	...
	if( !isset( $GLOBALS['g_'.$db_key] ) )
	{
		include_once(AROOT.'../config/conn.php');
		...
		$db_key = MD5( $host .'-'. $port .'-'. $user .'-'. $password .'-'. $db_name  );
		if( !$GLOBALS['g_'.$db_key] = @mysql_connect( $host.':'.$port , $user , $password , true ) )
		{
			//
		...
		else
		{
			if( $db_name != '' )
			{
				if( !@mysql_select_db( $db_name , $GLOBALS['g_'.$db_key] ) )
				{
		...
		@mysql_query( "SET NAMES 'UTF8'" , $GLOBALS['g_'.$db_key] );
	}
	return $GLOBALS['g_'.$db_key];
}
...
function last_error()
{
	if( isset( $GLOBALS['g_DB_LAST_ERROR'] ) )
	return $GLOBALS['g_DB_LAST_ERROR'];
}
